...
public class PBD_model: MonoBehaviour {
	...
	void Strain_Limiting()
	{
		...
		for (int i = 0; i < L.Length; i++)
		{
			int p_a = E[i * 2];
			int p_b = E[i * 2 + 1];
			Vector3 pos_a = X[p_a], pos_b = X[p_b];
			sum_X[p_a] += (pos_a + pos_b + (pos_a - pos_b) * L[i] / (pos_a - pos_b).magnitude) * 0.5f;
			sum_X[p_b] += (pos_a + pos_b - (pos_a - pos_b) * L[i] / (pos_a - pos_b).magnitude) * 0.5f;
			sum_N[p_a]++;
			sum_N[p_b]++;
		}
		for(int i = 0;i < X.Length; i++)
		{
			if (i == 0 || i == 20)
				continue;
			V[i] += ((0.2f * X[i] + sum_X[i]) / (0.2f + sum_N[i]) - X[i]) / t;
